...
class GPP4323(PowerSupply):
    ...
    def set_power_state(self, chan, state):
        # chan = 1,2,3,4
        # state = on, off
        self.visa_session.write(f":OUTP{chan}:STAT {state.upper()}")

    def set_voltage(self, chan, voltage):
        self.visa_session.write(f":SOUR{chan}:VOLT {voltage}")

    def get_voltage(self, chan):
        # chan = 1,2,3,4
        value =  self.visa_session.query(f":VOUT{chan}?")
        return float(value.strip().rstrip('V'))

    def set_current(self, chan, current):
        self.visa_session.write(f":SOUR{chan}:CURR {current}")

    def get_current(self, chan):
        # chan = 1,2,3,4
        value =  self.visa_session.query(f":IOUT{chan}?")
        return float(value.strip().rstrip('A'))

    def set_over_voltage_protection(self, chan, voltage):
        self.visa_session.write(f":OUTP{chan}:OVP {voltage}")
        self.visa_session.write(f":OUTP{chan}:OVP:STAT ON")

    def set_over_current_protection(self, chan, current):
        self.visa_session.write(f":OUTP{chan}:OCP {current}")
        self.visa_session.write(f":OUTP{chan}:OCP:STAT ON")
